export async function renderBuilderCard({ userImage, croppedAreaPixels, name, skill, title }) {
  ...
  // 4. Draw Text Overlay (Left-aligned next to the printed labels)
  // Name
  ctx.font = '700 42px "Space Mono"';
  ctx.fillStyle = '#FFFFFF';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText((name || 'YOUR NAME').toUpperCase(), 390, 1180);
  ...
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText((skill || 'YOUR SKILL').toUpperCase(), 370, 1365);
