import { drawCanvasFrame, drawCustomFrame } from './canvasFrames';
...
export async function renderFrame(canvas, {
  userImage, croppedAreaPixels,
  templateSrc, templateType, templateId,
  name, photoRadius, duotone, customSettings,
}) {
  ...
  // 3. Draw the template overlay
  if (templateType === 'canvas') {
    // Draw frame on a temp canvas, punch a transparent hole at the photo circle,
    // then composite so the dark bg goes around the photo and ring decorations
    // (which live outside the hole radius) layer on top of the photo.
    const tmpCanvas = document.createElement('canvas');
    tmpCanvas.width = SIZE;
    tmpCanvas.height = SIZE;
    const tCtx = tmpCanvas.getContext('2d');
    drawCanvasFrame(tCtx, templateId, SIZE);
    tCtx.save();
    tCtx.globalCompositeOperation = 'destination-out';
    tCtx.beginPath();
    tCtx.arc(CENTER, CENTER, PHOTO_RADIUS, 0, Math.PI * 2);
    ...
    tmpCanvas.height = SIZE;
    const tCtx = tmpCanvas.getContext('2d');
    drawCustomFrame(tCtx, SIZE, customSettings);
    tCtx.save();
    tCtx.globalCompositeOperation = 'destination-out';
    tCtx.beginPath();
    tCtx.arc(CENTER, CENTER, PHOTO_RADIUS, 0, Math.PI * 2);
